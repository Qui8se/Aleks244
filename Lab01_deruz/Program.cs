...
// 1. в компилируемом языке (С#) ошибки обнаруживаются еще до запуска программы на этапе компиляции. в интерпретируемом языке (Python) ошибки обнаруживаются во время выполнения программы, т.к. код выполняется построчно.

// 2. выполняет 2 действия 
// 1 - компилирует исходный код и создает исполняемые файлы 
// 2 - запускает полученный код на выполнение

// 3. bin - содержит готовые к запуску файлы программы
// obj - содержит временные файлы созданные во время компиляции. 

// 4. git commit это локальное операция, она сохраняет изменения на твоем компьютере 
// git push это сетевая операция, она отправляет все комиты на сервер (гитхаб).

// 5. конкатенация склеивает строки ("привет, " + name)
// интерполяция вставляет переменные прямо в строки ($"Привет, {name}").

// 6. при делении int на int дробная часть отбрасывается 
// нужно чтобы числа были с плавающей точкой (double) 

// 7. const это константа, ее значение задается один раз и ее нельзя изменить 
// пример число пи 
// const double pi = 3.14

// 8. Top-level statements - это синтаксис позволяющий писать код сразу в program.cs без необходимости создавать метод main 
// классический вариант требует объявления точки входа 
